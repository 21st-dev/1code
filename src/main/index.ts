...
import {
  ...
  type RunningDesktopMobileGateway,
} from "./lib/mobile-gateway/desktop"
import { setDesktopMobileGateway } from "./lib/mobile-gateway/desktop-state"
import {
  createMainWindow,
  createWindow,
  getWindow,
...
let mobileGatewayServer: RunningDesktopMobileGateway | null = null
...
if (gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    try {
      mobileGatewayServer = await startDesktopMobileGateway({
        enabled: IS_DEV || process.env.ONECODE_MOBILE_GATEWAY === "1",
      })
      setDesktopMobileGateway(mobileGatewayServer)
      if (mobileGatewayServer) {
        console.log(`[Mobile Gateway] Listening on ${mobileGatewayServer.url}`)
        if (IS_DEV) {
          console.log(
            `[Mobile Gateway] Dev pairing token: ${mobileGatewayServer.pairingToken}`,
          )
          const pairingUrl = mobileGatewayServer.getPairingUrl()
          if (pairingUrl) {
            console.log(`[Mobile Gateway] Dev pairing URL: ${pairingUrl}`)
          }
        }
      }
    } catch (error) {
      setDesktopMobileGateway(null)
      console.warn("[Mobile Gateway] Failed to start:", error)
    }
  ...
  app.on("before-quit", async () => {
    ...
    if (mobileGatewayServer) {
      await mobileGatewayServer.close()
      mobileGatewayServer = null
      setDesktopMobileGateway(null)
      console.log("[Mobile Gateway] Closed")
    }
