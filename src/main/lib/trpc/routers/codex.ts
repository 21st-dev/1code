...
function getSelectedCodexCliVersion(): string | null {
  try {
    return getCodexCliVersion(resolveCodexCliPath())
  } catch {
    return null
  }
}
...
export async function getCodexIntegrationStatus(): Promise<{
  ...
  return {
    ...
    version: getSelectedCodexCliVersion(),
  }
}
      ...
      return observable<any>((emit) => {
        ...
        ;(async () => {
          try {
            ...
            const providerRoute = input.authConfig
              ? `api-key:${input.authConfig.authMethodId ?? "codex-api-key"}`
              : effectiveAuthConfig
                ? "moss-provider"
                : "codex-cli"
            const buildCodexLaunchPlan = (params: {
              nativeSessionId?: string | null
              nativeSessionStrategy?: "start" | "resume"
              transport: string
              resultSubtype: "running" | "success" | "error" | "cancelled"
              nativeBridge?: string | null
              error?: string | null
            }) =>
              buildAgentRuntimeLaunchPlan({
                ...
                session: {
                  ...
                  runtimeConfigDir: join(homedir(), ".codex"),
                },
                nativeSessionStrategy: params.nativeSessionStrategy,
            ...
            if (shouldUseNativeCodexExec()) {
              ...
              const persistNativeRuntimeSession = (
                ...
                persistAgentRuntimeSession({
                  ...
                  launchPlan: buildCodexLaunchPlan({
                    nativeSessionId: latestNativeSessionId,
            ...
            const {
              ...
              providerInstanceId: input.providerInstanceId ?? null,
              modelSelection: input.modelSelection ?? null,
              fallbackModelId: input.model ?? DEFAULT_CODEX_MODEL,
            })
            ...
            try {
              persistAgentRuntimeSession({
                ...
                launchPlan: buildAgentRuntimeLaunchPlan({
                  ...
                  nativeSessionStrategy: latestKnownCodexSessionId
                    ? "resume"
                    : "start",
                  transport: "codex-chat",
                  providerRoute: input.authConfig ? "api-key" : "unknown",
                  resultSubtype: "error",
