...
export const MOSS_SESSION_ACTION_IDS = ["resume", "fork", "rollback"] as const;
...
export interface MossRollbackSnapshot {
  ...
  nativeRollbackTurnCount: number;
  mode: MossSessionActionMode;
  nativeSessionLinked: boolean;
}
...
function countUserTurnsAfterIndex(
  ...
  return messages
    .slice(targetIndex + 1)
    .filter((message) => messageRole(message) === "user").length;
}
...
function supportsNativeRollbackBridge(
  ...
  if (engine === "hermes") {
    return Boolean(
      nativeSessionId && hasTarget && hasFeature(features, "rollback"),
    );
  }
...
export function buildMossSessionActionPlan(
  ...
  const latestAssistantWithSdk =
    latestAssistantWithSdkIndex >= 0
      ...
      : undefined;
  const rollbackTargetIndex = latestAssistantWithSdk
    ? latestAssistantWithSdkIndex
    : latestAssistant
      ? latestAssistantIndex
      : latestMessageIndex;
  const rollbackTarget =
    rollbackTargetIndex >= 0 ? messages[rollbackTargetIndex] : undefined;
  const rollbackTargetRole = rollbackTarget
    ? messageRole(rollbackTarget)
    : undefined;
  ...
  const latestAssistantSdkMessageUuid = latestAssistant
    ...
    : undefined;
  const forkAtSdkUuid = latestAssistant
    ? (messageSdkUuid(latestAssistant) ?? null)
    : null;
  ...
  const nativeRollback =
    ...
    (input.engine !== "codex" || rollbackTargetRole === "assistant");
  ...
  const resume: MossSessionActionState = !hasFeature(input.features, "resume")
    ...
    : nativeSessionLinked
      ? {
          ...
          mode: "native",
          ...(input.engine === "codex"
            ? { nativeBridge: "codex-app-server-thread" as const }
          ...
          reason:
            input.engine === "codex"
              ? "Codex native session id is linked through app-server thread/resume."
              : input.engine === "hermes"
      ...
      : messages.length > 0
        ? {
            ...
            reason:
              "Transcript exists, but no native engine session has been linked yet.",
          }
        : {
            status: "unavailable",
  ...
  const fork: MossSessionActionState =
    messages.length === 0
      ? {
          status: "unavailable",
      ...
      : nativeFork
        ? {
            ...
            nativeBridge:
              input.engine === "codex"
                ...
                : input.engine === "hermes"
                  ...
                  : "claude-code-session",
            canRunHeadless:
              input.engine === "codex" || input.engine === "hermes",
            ...
            reason:
              input.engine === "codex"
                ? "Codex fork uses the app-server thread/fork bridge and keeps the native thread linked."
                : input.engine === "hermes"
        ...
        : {
            ...
            targetLabel: latestMessage
              ...
              : undefined,
            reason:
              "Moss will clone the transcript and start a fresh native engine session.",
          };

  const rollback: MossSessionActionState =
    messages.length === 0
      ? {
          status: "unavailable",
        ...
        : nativeRollback
          ? {
              ...
              reason:
                input.engine === "hermes"
                  ...
                  : input.engine === "codex"
                    ? "Codex rollback uses the app-server thread/rollback bridge and keeps the native thread linked."
          ...
          : messages.length > 1
            ? {
                ...
                targetLabel: messageLabel(rollbackTarget, "Rollback target"),
                reason:
                  "Moss will truncate the transcript and clear stale native session ids.",
              }
            : {
                status: "needs-target",
                mode: "message-history",
                reason:
                  "At least two messages are needed for message-history rollback.",
              };
...
function resolveForkCutoffIndex(
  ...
  if (targetMessageId) {
    const byId = messages.findIndex(
      (message) => messageId(message) === targetMessageId,
    );
...
export function buildMossForkSnapshot(
  ...
  const forkAtSdkUuid = lastAssistant
    ? (messageSdkUuid(lastAssistant) ?? null)
    : null;
  const nativeFork = supportsNativeForkBridge(
    input.engine,
    input.nativeSessionId,
...
function resolveRollbackTargetIndex(
  ...
  if (strictTarget && (targetMessageId || targetSdkMessageUuid)) return -1;
...
export function buildMossRollbackSnapshot(
  ...
  if (targetIndex < 0) {
    if (
      input.strictTarget &&
      (input.targetMessageId || input.targetSdkMessageUuid)
    ) {
      throw new Error(
        "Strict rollback target was not found in the Moss transcript.",
      );
    }
  ...
  const nativeRollbackTurnCount = countUserTurnsAfterIndex(
    messages,
    targetIndex,
  );
  const nativeRollback =
    ...
    (input.engine !== "codex" || messageRole(target) === "assistant");
...
export function mergeMossSessionControlMetadata(
  ...
  return JSON.stringify({
    ...parsed,
    mossSessionControl: {
      ...(isRecord(parsed.mossSessionControl) ? parsed.mossSessionControl : {}),
      ...controlMetadata,
