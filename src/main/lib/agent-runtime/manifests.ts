...
import type { AgentEngineId, AgentRuntimeManifest } from "./types";

const home = os.homedir();

export const AGENT_RUNTIME_MANIFESTS: Record<
  AgentEngineId,
...
> = {
  ...
  codex: {
    ...
    notes: [
      ...
      "Native fork, resume, rollback, thread management, config reads/writes, config requirements, permission profiles, model inventory, account snapshots, account login/logout, usage, rate limits, skill, hook, app, plugin, and MCP inventory, plus plugin install, external config migration, MCP config reload, and MCP OAuth login, are routed through the Codex app-server bridge.",
  ...
  hermes: {
    ...
    models: [{ id: "moss-default", label: "Moss Default" }],
    notes: [
  ...
  "custom-acp": {
    ...
    models: [{ id: "custom-acp", label: "Custom ACP Default" }],
    notes: [
...
export function getAgentRuntimeManifest(
