...
export function OpenLocallyDialog({
  ...
  const importMutation = trpc.sandboxImport.importSandboxChat.useMutation({
    onSuccess: (result) => {
      toast.success(t("agent.openLocally.opened"))
    ...
    onError: (error) => {
      toast.error(t("agent.openLocally.importFailed", { message: error.message }))
    },
  })
  ...
  const cloneMutation = trpc.sandboxImport.cloneFromSandbox.useMutation({
    onSuccess: (result) => {
      toast.success(t("agent.openLocally.clonedAndOpened"))
    ...
    onError: (error) => {
      toast.error(t("agent.openLocally.cloneFailed", { message: error.message }))
    },
  })
  ...
  const handleLocateProject = useCallback(async () => {
    ...
    if (result.success && result.project) {
      ...
      toast.error(
        t("agent.openLocally.wrongFolder", {
          found: result.found,
          expected: `${owner}/${repo}`,
        }),
      )
  ...
  const handleCloneFromSandbox = useCallback(async () => {
    ...
    // Clone
    toast.info(t("agent.openLocally.cloningRepository"))
              ...
              >
                {mode === "no-projects" ? (
                  ...
                  <>
                    <div className="p-6">
                      <h2 className="text-lg font-semibold mb-2">
                        {t("agent.openLocally.projectNotFound")}
                      </h2>
                      <p className="text-sm text-muted-foreground mb-5">
                        {t("agent.openLocally.missingProjectPrefix")}{" "}
                        <code className="px-1.5 py-0.5 bg-muted rounded text-foreground text-xs">
                          {repository}
                        </code>
                        {t("agent.openLocally.missingProjectSuffix")}
                      </p>
                      ...
                      <div className="space-y-2">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="text-sm font-medium">
                                {t("agent.openLocally.haveItCloned")}
                              </div>
                              <div className="text-xs text-muted-foreground">
                                {t("agent.openLocally.localCopyHint")}
                              </div>
                            </div>
                          </div>
                        </button>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="text-sm font-medium">
                                {t("agent.openLocally.cloneFromSandbox")}
                              </div>
                              <div className="text-xs text-muted-foreground">
                                {t("agent.openLocally.downloadRepository")}
                              </div>
                            </div>
                          </div>
                        </button>
                      ...
                      {isAnyLoading && (
                        <div className="mt-4 text-xs text-muted-foreground text-center">
                          {locateMutation.isPending &&
                            t("agent.openLocally.openingFolderPicker")}
                          {pickDestMutation.isPending &&
                            t("agent.openLocally.openingFolderPicker")}
                          {importMutation.isPending &&
                            t("agent.openLocally.importing")}
                          {cloneMutation.isPending &&
                            t("agent.openLocally.cloningRepositoryShort")}
                        </div>
                      )}
                    </div>
                  ...
                  <>
                    <div className="p-6">
                      <h2 className="text-lg font-semibold mb-2">
                        {t("agent.openLocally.multipleCopies")}
                      </h2>
                      <p className="text-sm text-muted-foreground mb-5">
                        {t("agent.openLocally.multipleCopiesPrefix")}{" "}
                        <code className="px-1.5 py-0.5 bg-muted rounded text-foreground text-xs">
                          {repository}
                        </code>{" "}
                        {t("agent.openLocally.multipleCopiesSuffix")}
                      </p>
                    ...
                    <div className="bg-muted/50 px-6 py-4 flex justify-between border-t border-border">
                      ...
                      >
                        {importMutation.isPending
                          ? t("agent.openLocally.opening")
                          : t("agent.openLocally.openLocally")}
                      </Button>
                    </div>
                  </>
                )}
