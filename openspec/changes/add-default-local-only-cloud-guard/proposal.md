...
## Why
The open-source desktop app still has several code paths that contact hosted 1Code/21st services by default. Local-first users need a centralized guard that makes official cloud access opt-in rather than relying on scattered UI hiding.

## What Changes
- Default the app to local-only mode, with an explicit environment override for development or internal builds.
- Block official 1Code/21st hosted APIs, CDN updates, remote sandbox, hosted auth, hosted voice/TTS, analytics, and error reporting while local-only mode is enabled.
