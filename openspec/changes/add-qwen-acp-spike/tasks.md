...
## 0. Pre-flight
- [ ] 0.1 Land/stash `refactor-first-run-onboarding`; branch
...
- [ ] 0.3 Define smoke isolation before auth: use isolated HOME/Qwen config and
...
- [ ] 1.1 Add a shared `qwen-code` runtime feature flag (off by default)
- [ ] 1.2 Add a narrower non-desktop contract runtime constant, e.g.
      `CONTRACT_RUNTIME_IDS = ["claude-code", "codex"]`.
- [ ] 1.3 Move non-desktop contract consumers from `AGENT_RUNTIME_IDS` to
      `CONTRACT_RUNTIME_IDS`: Local Job API (`local-job-api.ts`), headless CLI
      parsing, schedules, headless job store, and `locus acp`.
- [ ] 1.4 Add one focused guard test proving `CONTRACT_RUNTIME_IDS` excludes
      `qwen-code` and the public non-desktop schema/parser surfaces do not accept
      Qwen in this spike.
- [ ] 1.5 Thread the Qwen flag only through desktop spike surfaces: manifest
...
- [ ] 2.1 Add `"qwen-code"` to the known runtime ID owner and alias `"qwen"`
      only after the static contract split is in place.
- [ ] 2.2 Extend `DesktopRuntimeAdapterSource` with `"qwen-acp-client"`
      (`src/main/lib/agent-runtime/desktop-runner.ts:9`).
- [ ] 2.3 Replace the hard allowlist at `desktop-runner.ts:97` with a desktop
      Qwen-flag-aware admit check; keep default builds rejecting `qwen-code`.
- [ ] 2.4 Extend `DesktopPermissionRuntime` to include `"qwen-code"` only through
      desktop Qwen flag-aware policy resolution
      (`src/main/lib/agent-runtime/permission-policy.ts:9`).
- [ ] 2.5 Unit tests: with flag off, desktop factory and permission layer reject
      `qwen-code`; with flag on, desktop-only Qwen admission works.

## 3. Desktop chat entry
- [ ] 3.1 Add a narrow runtime-neutral desktop chat subscription route, such as
      `agentRuntime.chat.subscribe`, for the Qwen spike.
- [ ] 3.2 Keep the route envelope-only: validate input, check enabled runtime,
...
- [ ] 3.3 Do not migrate Claude or Codex desktop chat in this slice; existing
      `claude.chat` and `codex.chat` subscriptions remain unchanged.
- [ ] 3.4 Add focused route tests for envelope validation, flag-off rejection, and
...
- [ ] 4.1 New local ACP client transport module in `src/main/lib/agent-runtime/`
      that can launch a configured command/args pair and speak ACP over stdio.
- [ ] 4.2 Configure Qwen with `command: "qwen"`, `args: ["--acp"]`; do not use
      `qwen serve`, HTTP `/acp`, or remote HTTP+SSE in this change.
- [ ] 4.3 Lifecycle: spawn, initialize/ready, graceful shutdown, crash/exit
      handling, stderr redaction.
- [ ] 4.4 Cancellation: Locus cancel -> ACP cancel -> terminate without orphaned
      process.
- [ ] 4.5 Review check: the generic stdio transport imports nothing Qwen-specific
...
- [ ] 5.1 New `src/main/lib/qwen/` adapter mirroring
      `createCodexAppServerAdapter`
      (`src/main/lib/codex/app-server-adapter.ts:326`), returning a
      `DesktopRuntimeAdapter`.
- [ ] 5.2 Map ACP session/stream/tool/permission events to Locus run events +
      `DesktopRunResult`.
- [ ] 5.3 Map ACP errors to Locus error events; unknown ACP events produce a
      single "unsupported event" diagnostic (never crash).
- [ ] 5.4 Pass project MCP config through to Qwen; record what it accepts.
- [ ] 5.5 Register the adapter with the factory under
      `qwen-code:qwen-acp-client`

## 6. Conservative permission tier
- [ ] 6.1 Add `QwenPermissionMapping` modeled on
      `CodexAppServerPermissionMapping` (`permission-policy.ts:50`): approval
      gate required, fail-closed when hook unavailable.
- [ ] 6.2 Route Qwen file-write / shell / MCP tool calls through Locus guard +
      trace; unmapped tools -> `unknown-tool`.
- [ ] 6.3 Test: a denied permission request fails closed and is traced.
- [ ] 6.4 Test: missing/delayed Qwen approval hook fails closed before side
      effects execute.

## 7. Honest capability manifest
- [ ] 7.1 Author a `qwen-code` manifest - only wired capabilities `supported`;
      everything else `degraded`/`unsupported` with honest reasons.
- [ ] 7.2 Ensure renderer capability gates read the manifest (no new hardcoded
      Qwen branches beyond edge transport routing).
- [ ] 7.3 Ensure manifest, diagnostics, and smoke notes contain no API keys,
      OAuth tokens, raw headers, or secret-bearing env values.

## 8. Minimal renderer and chat metadata touches
- [ ] 8.1 Add Qwen to shared provider/runtime metadata
      ...
      feature flag.
- [ ] 8.2 Add a thin runtime-neutral desktop chat transport for Qwen that calls
      the new `agentRuntime.chat.subscribe` route.
- [ ] 8.3 Add a flag-gated runtime/provider option in `new-chat-form.tsx`.
- [ ] 8.4 Wire Qwen question/approval state through the existing
      `runtime-event-state.ts` owner; do not add renderer-local Qwen approval
      business logic.
- [ ] 8.5 Explicitly NOT touching the broad hardcoded Claude/Codex renderer
...
- [ ] 9.5 Error mapping: a forced failure maps to a Locus error, not a
      hang/crash.
- [ ] 9.6 MCP config passthrough verified or explicitly documented as degraded.
- [ ] 9.7 Flag-off smoke/static guard: default desktop remains Claude Code +
      Codex only, and non-desktop contract surfaces remain statically Claude
      Code + Codex only.
- [ ] 9.8 Write spike findings: what generalized cleanly, what the follow-up
      renderer manifest work must refactor, and whether a later `qwen serve`
      HTTP `/acp` transport is worth proposing.

## 10. Validate
- [ ] 10.1 `openspec validate add-qwen-acp-spike --strict --no-interactive`
- [ ] 10.2 Typecheck + targeted tests green; flag-off smoke shows unchanged
