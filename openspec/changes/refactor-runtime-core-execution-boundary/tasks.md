...
- [ ] 7.3 Run targeted tests plus `bunx openspec validate --all --strict --no-interactive` before implementation handoff.
- [ ] 7.4 Confirm implementation preserves existing `agent-scope-contracts` and `runtime-security-baseline` behavior unless a follow-up proposal changes them.
