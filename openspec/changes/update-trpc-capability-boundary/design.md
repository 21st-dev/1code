...
|---|---|---|---|
| Critical | `terminal.createOrAttach` (`terminal.ts:15`), `terminal.write` (`terminal.ts:46`), `terminal.signal` (`terminal.ts:72`), `terminal.kill` (`terminal.ts:86`) | Starts and controls a PTY; accepts renderer `cwd` and `initialCommands`, then arbitrary bytes to the terminal. | Needs registered workspace/cwd resolution and capability/consent. |
...
| High | `projects.cloneFromGitHub` (`projects.ts:249`) | Uses child-process shell command to run `git clone` after parsing renderer `repoUrl`. | Should use argv-based git execution and constrained repo identity. |
...
| High | `files.search/watchChanges/renameFile/deleteFile` (`files.ts:388`, `:568`, `:654`, `:680`) | Reads directory trees, watches paths, renames, or trashes filesystem paths. | `readFile/readTextFile/readBinaryFile` now use registered roots; these adjacent path sinks still need the same root semantics. |
