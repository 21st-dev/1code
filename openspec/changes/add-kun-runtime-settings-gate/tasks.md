...
- [ ] 0.2 Inventory every Kun enablement read path (resolver, registry,
...
- [ ] 1.1 New main-process `src/main/lib/agent-runtime/runtime-feature-settings.ts`:
...
- [ ] 1.2 Resolver mode = `app.isPackaged`: packaged (product) ⇒ gate = persisted
...
- [ ] 1.3 Inject the resolved state from main into the registry through an explicit
...
- [ ] 1.4 Test: in product mode the resolver ignores `LOCUS_ENABLE_KUN_RUNTIME=1`
      (Kun stays hidden); in dev/test mode env still enables Kun (keeps the existing
      env-based tests + live-smoke harness working).

## 2. tRPC
- [ ] 2.1 Add `agentRuntime.getRuntimeFeatureSettings` and
      `agentRuntime.setKunRuntimeEnabled({ enabled })`.
- [ ] 2.2 Kun chat / install / approve-shell / config tRPC fail closed when the
      setting is off, regardless of saved state or env.
- [ ] 2.3 On `setKunRuntimeEnabled(false)`: abort active Kun streams and deny/clear
...
## 3. Settings UI
- [ ] 3.1 Add an experimental/advanced area in Settings > Models with an
      off-by-default "Enable Kun runtime" toggle (no promotional copy).
- [ ] 3.2 When off, show only the toggle; hide the Kun CLI / shell-approve /
      managed-install blocks. When on, surface them.
- [ ] 3.3 The toggle mutation invalidates `agentRuntime.listManifests` /
...
- [ ] 4.1 Filter `kun` out of profile targets at BOTH paths when disabled: the
...
- [ ] 4.2 Enforce the same rule at the main save boundary
...
- [ ] 4.3 Disabling Kun does not delete the saved executable path / config path /
      shell-approved hash; re-enabling restores usability.

## 5. Acceptance (hard lines)
- [ ] 5.1 Product build, setting off (default): Kun absent from Engine list,
      manifest, Settings (only toggle), and profile targets — even with
      `LOCUS_ENABLE_KUN_RUNTIME=1` set.
- [ ] 5.2 Kun chat/install/approve-shell/config fail closed when off (called
      directly).
- [ ] 5.3 Toggle on → Kun surfaces and works (BYO); toggle off → hidden but saved
      state + profiles preserved; toggle back on → usable from preserved state.
- [ ] 5.4 Qwen enablement/exposure unchanged; dev/test env path still enables Kun
      for the existing tests and smoke harness.
- [ ] 5.5 Disabling Kun during a pending Kun approval aborts the stream + denies the
      pending approval; a later `respondToolApproval` for it fails closed.
- [ ] 5.6 Applying a `kun`-bearing preset while disabled produces a profile without
...
- [ ] 5.7 Toggling off invalidates the manifest/Kun-status queries so the UI drops
      Kun immediately (no 60s stale window).

## 6. Validate
- [ ] 6.1 `openspec validate add-kun-runtime-settings-gate --strict --no-interactive`.
- [ ] 6.2 `bun run check` green (tests that set the env flag still enable Kun via
      the dev/test path).
