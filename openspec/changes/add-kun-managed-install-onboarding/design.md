...
- **Fixed allowlist, not renderer or dynamic latest:** the renderer can call
  ...
  determined in main process from platform/arch and the allowlist.
...
- Which Kun asset should be enabled in the production allowlist first? The initial
  implementation can enable macOS zip assets with pinned SHA-256 and leave other
  platforms as unsupported until their extraction/launch paths are proven.
