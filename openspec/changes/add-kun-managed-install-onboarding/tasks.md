...
- [x] 2.1 Add `src/main/lib/kun/kun-managed-install.ts` as the only owner for
...
- [x] 2.2 Use only main-process allowlisted build metadata: version, platform,
      arch, asset URL, asset name, SHA-256, size, archive kind, executable path.
- [x] 2.3 Install under `{userData}/runtimes/kun/<version>/...`; never install to
      `/usr/local/bin`, never mutate PATH or shell profile.
- [x] 2.4 Download to an app-managed temporary path; verify SHA-256 before
      extraction/install; cleanup staging on success and failure.
- [x] 2.5 Reject archive entries that escape staging/install roots; chmod final
...
- [x] 3.2 Add tRPC mutations for managed install/update with no renderer-provided
      URL/checksum/shell command input.
- [x] 3.3 Existing BYO executable/config path routes remain available and
      unchanged.

## 4. Settings/onboarding UI
- [x] 4.1 Add `Install Kun` / `Update Kun` controls only when an allowlisted build
      is available; show guided BYO when unavailable.
- [x] 4.2 Distinguish Not installed, Installed but no config, Installed and config
      ready, Shell not approved, Shell approved, Update available, and Hash
      mismatch shell disabled.
- [x] 4.3 Do not present installed as shell supported; keep the existing explicit
      shell approval action.

## 5. Tests
- [x] 5.1 Renderer cannot inject download URL, checksum, archive path, shell
      command, or destination path into install/update routes.
- [x] 5.2 Checksum mismatch fails closed, deletes staging, and leaves previous
      executable settings untouched.
- [x] 5.3 Zip path traversal and absolute-path entries are rejected.
- [x] 5.4 Successful install lands inside managed dir, chmods executable, and
      saves only the final managed executable path.
- [x] 5.5 Updating to a different binary hash makes existing shell approval
