...
- [x] 1.1 Inventory every user-facing i18n key (en + zh-CN) referring to an entity
  ...
  Chat entity.
- [x] 1.2 For each **create action**, determine what it creates by reading behavior
...
- [x] 1.3 Inventory the scattered empty-state / onboarding strings ("Welcome to
  Agents", "No project open", "No projects found", "No projects", "Attach folder")
  and map each to the §5 entry grammar.
- [x] 1.4 List the small set of **misleading handlers/identifiers** to rename (e.g.
  `handleNewAgent`), confirming none are part of the §7 protected surface
  (`subChat*`, `job`/`local-job-api`, DB tables).
- [x] 1.5 Confirm the existing `workspace-navigation`, `ui-localization`, and
...
- [x] 2.1 Correct create-action labels to the canonical term for what they create
...
- [x] 2.2 Rename the misleading handlers to match behavior (e.g. `handleNewAgent` →
  `handleNewQuickChat`) and update call sites; no behavior change.
- [x] 2.3 Sweep remaining user-facing create/label strings so "Agent" never appears
  on a workspace/chat-create control and the worktree layer reads "Workspace".

## 3. Part ④ — unified entry / empty-state language

- [x] 3.1 Replace the multiple empty-state phrasings with the §5 grammar (Open a
...
- [x] 3.2 Align "Attach folder" → "Attach a Project" (quick-chat graduation) per §4/§5.

## 4. Guard against re-drift

- [x] 4.1 Add a bounded i18n guard (in `scripts/check-architecture-guards.mjs` or a
  ...
  wording.
- [x] 4.2 Confirm the guard passes on the cleaned dictionaries and fails on a
...
- [x] 5.3 `bun run lint` (changed-line biome) green.
- [x] 5.4 Run the architecture guard incl. the new i18n assertion.
- [x] 5.5 `openspec validate refactor-canonical-vocabulary --strict --no-interactive`.
- [x] 5.6 Confirm the §7 boundary held: no `subChatId`/`SubChat` rename, no `job`/
  `local-job-api` change, DB tables untouched; only i18n values + named handlers moved.
- [x] 5.7 Visual/source smoke: after restarting the dev app, sidebar create actions
  and the composer project selector read in one consistent vocabulary in zh-CN;
  onboarding copy is covered by i18n tests and the architecture guard.
- [x] 5.8 Mark `docs/ideas/canonical-vocabulary.md` §4/§5 implemented.
