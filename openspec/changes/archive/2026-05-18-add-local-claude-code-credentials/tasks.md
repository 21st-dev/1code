...
- [x] 5.4 Smoke-test local-only startup with no desktop login, import local Claude Code credentials, send a simple read-only task, and confirm no 21st hosted auth requests are made. Local-only startup, bundled CLI versions, Codex login status, and local Claude Code credential probing were verified. App-level active `Local Claude Code` credentials were then validated through the dev app database using Electron safeStorage, and a read-only Claude Code SDK task completed without hosted 21st/auth/sandbox URL markers in smoke output.
