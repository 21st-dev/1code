...
- [x] 4.2 Reframe visible copy and navigation as Runs/History or Job Trace for
  headless/API/daemon jobs and historical audit.
- [x] 4.3 Keep actions that open linked chats, diff review, and PR flows wired to
  existing safe local behavior.
- [x] 4.4 Render the selected job's record header (runtime, provider profile or
...
- [x] 5.1 Route remaining direct Plan, Diff, and Terminal product entry points
  through DetailsSidebar widgets before opening expanded renderers.
- [x] 5.2 Migrate remaining independent right-sidebar controls one at a time
  using the existing hidden-button and expanded-widget patterns.
...
## 6. Validation
- [x] 6.1 Run targeted presenter and renderer tests.
- [x] 6.2 Run `bun run ts:check` if implementation touches TypeScript.
- [x] 6.3 Run `openspec validate refine-chat-first-run-trace-workbench --strict --no-interactive`.
- [x] 6.4 Update `docs/DESIGN.md`, `docs/run-event-trace-inventory.md`, or
