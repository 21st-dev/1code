...
## 0. Pre-flight
- [ ] 0.1 Branch off clean `main`.
- [ ] 0.2 Inventory the three-way second control on both surfaces
...
- [ ] 1.1 Build an Engine control listing enabled runtimes (claude-code, codex,
...
- [ ] 1.2 Make it the ONLY runtime-switch entry; remove runtime switching from the
      model selector.
- [ ] 1.3 Move the `CrossProviderConfirmDialog` from `agent-model-selector.tsx`
      onto the Engine control (switch confirm now owned here).
- [ ] 1.4 Enforce setup-required/unavailable semantics: those engines are
...
- [ ] 1.5 Tests: selecting a ready engine changes the runtime; status renders from
...
- [ ] 2.1 Build one Model component parameterized by the selected Engine; render
      that runtime's sources/models/provider-profiles/BYO with identical shape
      across runtimes.
- [ ] 2.2 Show contextual options (Codex thinking + account source, Claude
      thinking) ONLY when the selected engine/model supports them.
- [ ] 2.3 Back the selected value per-engine via the existing
      `lastSelected{Claude,Codex,Kun}ModelSource` / `lastSelectedModelId` atoms;
      switching Engine swaps the Model control's value.
- [ ] 2.4 For active-chat, read/write the current sub-chat atom families
...
- [ ] 2.5 Preserve Claude/Codex internals by RELOCATION not rewrite: model list,
      thinking submenus, Codex account-source control + compatibility resolution,
      model-info panels, Ollama path.
- [ ] 2.6 The Model control never changes the runtime (guard/test).

## 3. Runtime-managed + Kun unification
- [ ] 3.1 Qwen renders a runtime-managed / setup state in the Model control (no
      provider-profile rows; `providerProfiles` is `degraded`); no secret reaches
      the renderer.
- [ ] 3.2 Replace `KunProviderProfileSelector`'s bespoke role with the shared
...
- [ ] 4.1 Widen `handleContinueWithProvider` / `handleInputProviderChange` from
      `"claude-code" | "codex"` to all four runtimes.
- [ ] 4.2 Preserve the empty active sub-chat fast path: switching Engine in an
      empty sub-chat updates that sub-chat in place, without history attachment or
      creating a new tab.
- [ ] 4.3 Generalize per-runtime model-preference inheritance in the new-sub-chat
...
- [ ] 4.4 Tests: switching into each runtime (claude/codex/qwen/kun) from an empty
...
- [ ] 5.1 Wire Engine + Model into both `new-chat-form.tsx` and
      `chat-input-area.tsx`.
- [ ] 5.2 Guard/test: both surfaces use the same components with consistent
      behavior (no drift).
- [ ] 5.3 Add English and Simplified Chinese i18n entries for Engine, Model,
...
- [ ] 6.1 Claude: model selection, thinking toggle, Ollama/offline path, provider
      profile selection — unchanged on both surfaces.
- [ ] 6.2 Codex: model selection, thinking level, ChatGPT/API-key account source,
      source/model compatibility, provider profile selection — unchanged.
- [ ] 6.3 Existing provider-routing-ux Codex requirements (account source
      selection, source/model compatibility) stay green.
- [ ] 6.4 Kun: works through the shared Model control, no bespoke selector; Qwen:
      runtime-managed state, no fabricated provider-profile rows.
- [ ] 6.5 Setup-required/unavailable Qwen/Kun cannot start runs or create
      sub-chats; setup actions route to Settings.
- [ ] 6.6 Engine switch behavior works for all four runtimes: empty active
      sub-chat switches in place; non-empty active chat confirms and creates a new
      sub-chat with history.

## 7. Validate
- [ ] 7.1 `openspec validate refactor-agent-engine-model-controls --strict --no-interactive`.
- [ ] 7.2 `bun run check` green; renderer handles only safe source/profile ids
