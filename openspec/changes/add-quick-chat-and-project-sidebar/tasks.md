...
- [x] 1.5 Audit repo-only read paths (`getFileStats`, worktree/diff/terminal derivation, PR actions, workbench job queries, kanban, MCP/project lookup) and exclude or special-case null-project chats.
...
- [x] 4.4 Gate quick-chat provider/model choices to runtimes whose assistant enforcement is supported by capability truth.
...
- [x] 5.5 Update deferred repository onboarding so folderless quick-chat creation is available without a project while project-dependent repository workflows remain unavailable.
...
- [x] 8.2 For code workspaces keep archive primary, delete behind a confirm dialog, with stronger confirmation when the worktree has uncommitted changes or an open PR.
...
- [x] 10.1 Add English and Simplified Chinese strings for quick chat, assistant mode, open repository, attach folder, permanent delete / clear, and confirm dialogs.
...
- [x] 10.5 Manual smoke and hardening evidence: create quick chats; verify Claude assistant policy probes plus SDK `disallowedTools` for known non-web tools; verify Codex ACP/app-server file/shell/MCP requests fail closed before execution; upload a file -> rewrite + download; attach a folder -> fresh project-backed session with full tools; sidebar groups by project with quick chat on top; delete a quick chat one-click; archive vs delete a code workspace. A live Claude provider `Read` tool-call attempt is not claimed unless separately recorded in `smoke-evidence.md`.
