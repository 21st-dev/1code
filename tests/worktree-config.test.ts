...
describe("worktree config detection", () => {
  test("uses custom path before built-in config locations", async () => {
    ...
    await writeJson(join(project, "custom.json"), { "setup-worktree": ["custom"] })
  ...
  test("prefers .locus over .cursor and legacy .1code", async () => {
    const project = await makeProject()
    await writeJson(join(project, ".locus/worktree.json"), { "setup-worktree": ["locus"] })
    await writeJson(join(project, ".cursor/worktrees.json"), { "setup-worktree": ["cursor"] })
    await writeJson(join(project, ".1code/worktree.json"), { "setup-worktree": ["legacy"] })
