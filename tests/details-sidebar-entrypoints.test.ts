...
describe("details sidebar entrypoints", () => {
  ...
  test("promotes Details changes commit and push without duplicating git owners", () => {
    ...
    expect(changesWidget).toContain("freshGitStatus")
    expect(changesWidget).toContain("refetchOnWindowFocus: true")
    expect(changesWidget).toContain("statusFilesToParsedDiffs")
    expect(changesWidget).toContain("mergeDisplayFiles")
    expect(detailsSidebar).toContain("unstaged={gitStatus?.unstaged ?? []}")
